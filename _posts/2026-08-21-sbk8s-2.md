---
title:  "Releasing Spring Boot Applications on Kubernetes (2)"
categories:
  - Java
  - Spring Boot 4
  - Kubernetes
...
tags:
  ...
  - DZone
toc: true
last_modified_at: 2026-08-21T08:05:34-05:00
---
The [previous post](https://www.simplex-software.fr/posts-archive/sbk8s/) in this series demonstrates how to deploy and run Spring Boot
applications on K8s without having to manually craft YAML files. This 2nd part
takes it one step further and serves the very same `/hello/{who}` endpoint, but
over HTTPS on port 8443, using an X.509 certificate that is provisioned, renewed
and rotated automatically by [cert-manager](https://cert-manager.io/).It requires
`docker` and `minikube` installed locally, as well as having a docker hub account.

You can find the project in the same [GitHub repository](https://github.com/nicolasduminil/sb-k8s.git), on the `cert-manager` branch.

Instead of manually running `kubectl create deployment` and `kubectl expose`, as
we did previously, this branch describes the whole desired state as a set of 
YAML manifests (under `k8s/`) and lets [Skaffold](https://skaffold.dev/) build the image and apply the
manifests in one shot.

## Managing Certificates with `cert-manager`

In order to setup HTTPS connections to our endpoint we need to generate an X509
certificate for it such that any client be able to validate the service identity.
Otherwise a malicious service might impersonate the ours and causing annoiances.
The primary tools for dealing with X509 certificates are `openssl` and `keytool`.
Both offer commands for creating public/private key pairs as well as signing and
converting keys and certificates.

Certificates have an expiration date which, once exceeded, they become automatically
lapsed and, hence, should be replaced by new ones. This process is known as 
*certificate rotation* and is a critical one, enterprise-wide, as it's quite easy
to imagine the effects of expired certificates preventing applications and services
to communicate. hence the importance of automating this process.

With Kubernetes, this automation cand be achived using `cert-manager`, a native
tool for managing X.509 certificates. Among its main automated functionalities:

  - obtaining X.509 certificates from the most popular public issuers;
  - validating their activation period as being between their associated `notBefore` and `notAfter` timestamps;
  - renewing them before they expire.

A Kubernetes operator able to generate, rotate and save certificates as secrets
is available and can be installed via its official Helm chart, as we will show
in a moment.

## Handling K8s manifest files with `skaffold`

`skaffold`, as defined by its maintainers in the official GIT [repository](https://github.com/GoogleContainerTools/skaffold.git),
is "a command line tool that facilitates continuous development for Kubernetes 
applications". It can be used to build and deploy K8s controllers while avoiding
having to handle individual YAML files. 

A `skaffold.yaml` file groups all the other YAML files a cluster needs to handle.
In our case, given that we need to deal with a half dozen of YAML files that 
define the deployment, the service, the certificate, the configmap, the secrets, etc.
it is much easir to group all these files in a `skaffold.yaml` one and perform
the required operation as described [here](https://skaffold.dev/docs/install/).
...
installed in the cluster (see below).

## Building, deploying and running

In order to build and deploy the project proceed as documented below:
...
not to verify the server's certificate:
    ...
    $ curl -k https://localhost:8443/hello/nicolas
    Hello nicolas
...
secret, so you can extract it and tell `curl` to trust it:
    ...
    $ curl --cacert ca.crt https://localhost:8443/hello/nicolas
    Hello nicolas
...
that CA into your operating system's trust store once:
    ...
    $ curl https://localhost:8443/hello/nicolas        # no -k, no --cacert
    Hello nicolas
...
Because this issuer in this example is self-signed, the walkthrough stops here: `ca.crt`
...
stable CA that outlives the leaf certificates. A next post will address this point.
