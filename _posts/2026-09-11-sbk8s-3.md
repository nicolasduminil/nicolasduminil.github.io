---
title:  "Releasing Spring Boot Applications on Kubernetes (3)"
categories:
  - Java
  - Spring Boot 4
  - Kubernetes
...
tags:
  ...
  - DZone
toc: true
last_modified_at: 2026-09-11T08:05:34-05:00
---

The [previous post](https://www.simplex-software.fr/posts-archive/sbk8s-2/) in this series demonstrates how to deploy and run Spring Boot
applications on K8s, using one way TLS and a X509 certificate that is provisioned,
renewed and rotated automatically by `cert-manager`. While this scenario provides
advanced functionalities and unquestionable advantages, it presents an issue: 
*anyone* who can reach the socket gets served. 

This 3nd part turns on mutual TLS (mTLS), so the server also
...
You can find the project in the same [GitHub repository](https://github.com/nicolasduminil/sb-k8s.git), on the `mtls` branch.

## A CA hierarchy is required
...
validate a client certificate. This 3rd part therefore introduces a tiny in-cluster
...
stays in the `default` namespace, as opposed to the CA `ClusterIssuer`, used 
previously, that would have expected it in `cert-manager`'s cluster-resource 
namespace instead.
...
waive the server's `client-auth = need` requirement:
    ...
    $ curl -k --cert client.crt --key client.key https://localhost:8443/hello/nicolas
    Hello nicolas
...
`localhost` is accepted:
    ...
    $ curl --cacert ca.crt --cert client.crt --key client.key https://localhost:8443/hello/nicolas
    Hello nicolas
...
system's trust store once (you still need `--cert`/`--key` for client auth):
    ...
    $ curl --cert client.crt --key client.key https://localhost:8443/hello/nicolas
    Hello nicolas
...
The property that makes this scenario interesting is its stable CA. The server
leaf is signed by `ca-issuer`, which is backed by the long-lived `sb-k8s-ca`
certificate (see [A CA hierarchy is required](#a-ca-hierarchy-is-required)).
...
across a rotation, and it is the difference from the previous self-signed `cert-manager`
scenario, where `ca.crt` rotates together with the leaf and this whole walkthrough is
impossible.
...
5. Go to do something else and come back after an hour. `cert-manager` rotated
   ...
   still serving the certificate that it has read at startup, which has now expired, so the same call fails:
...
both commands still reported it valid, now, once it expires, they flip as shown above.
